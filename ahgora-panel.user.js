...
(function () {
    ...
    function calcularTrabalhado(batidas) {
        ...
        for (let i = 0; i < batidas.length; i += 2) {
            ...
            let saida = batidas[i + 1] ? toMin(batidas[i + 1]) : nowMin();
            total += (saida - entrada);
        }
        return total;
    }
    ...
    function notif(id, title, body, urgente = false) {
        ...
        try {
            new Notification(title, { body, requireInteraction: urgente, tag: id });
        } catch (e) {
            console.error(e);
        }
    }
    ...
    function extrairDados() {
        ...
        dias.forEach(day => {
            ...
            resultado.push({ data, isToday, isFuture, isHoliday, isBusinessDay, batidas, trabalhado, saldo });
        });
    ...
    function calcularResumo() {
        ...
        const retorno11h = ultimaBatida !== null ? ultimaBatida + (11 * 60) : null;
        ...
        if (hoje.batidas.length >= 2) {
            const trabalhadoTurno1 = toMin(hoje.batidas[1]) - toMin(hoje.batidas[0]);
        ...
        if (hoje.batidas.length >= 1) {
            const e1 = toMin(hoje.batidas[0]);
            const s1 = hoje.batidas[1] ? toMin(hoje.batidas[1]) : nowMin();
            turno1 = {
                entrada: hoje.batidas[0], saida: hoje.batidas[1] || 'agora', aberto: !hoje.batidas[1], total: s1 - e1, limite: CONFIG.MAX_HORAS_TURNO,
        ...
        if (hoje.batidas.length >= 3) {
            const e2 = toMin(hoje.batidas[2]);
            const s2 = hoje.batidas[3] ? toMin(hoje.batidas[3]) : nowMin();
            turno2 = {
                entrada: hoje.batidas[2], saida: hoje.batidas[3] || 'agora', aberto: !hoje.batidas[3], total: s2 - e2, limite: CONFIG.MAX_HORAS_TURNO,
        ...
        const status = (() => {
            const qtd = hoje.batidas.length;
            if (qtd === 0) return '🛬 Não iniciado';
            if (qtd === 1) return '🥇 Primeiro turno';
            if (qtd === 2) return '⏸ Intervalo';
            if (qtd === 3) return '🥈 Segundo turno';
            if (qtd >= 4) return '🛫 Encerrado';
            return '--';
        })();
        ...
        let alerta = null;
        if (hoje.batidas.length >= 2) {
            if ((toMin(hoje.batidas[1]) - toMin(hoje.batidas[0])) > CONFIG.MAX_HORAS_TURNO) alerta = '⚠️ Primeiro turno excedeu 6h';
        }
        if (hoje.trabalhado > CONFIG.MAX_HORAS_DIA) alerta = '⚠️ Limite diário excedido';

        return { hoje, saldoSemana, saldoMes, totalMes, dias, diasRestantesMes, diasRegistrados, entrada, turno1, turno2, retorno11h, h6, h8, h10, saidaIdeal, status, retornoMinimo, retornoMaximo, trabalhado: dias.trabalhado, alerta };
    }
    ...
    function injectCSS() {
        ...
        style.id = 'ahg-css-v5';
        style.textContent = `
        #ahg-fab{ position:fixed; bottom:20px; right:20px; left: auto; z-index:99999; width:48px; height:48px; border-radius:50%; background:linear-gradient(135deg,#3b2d82,#1e1b4b); border:2px solid #4a3faf; display:flex; align-items:center; justify-content:center; font-size:22px; cursor:pointer; color:white; }
        #ahg-eye-fab { position: fixed; bottom: 80px; right: 20px; width: 48px; height: 48px; border-radius: 50%; background: linear-gradient(135deg, #2b2b40, #1a1a2e); border: 2px solid #33334d; display: flex; align-items: center; justify-content: center; font-size: 20px; cursor: pointer; color: white; z-index: 99998; opacity: 0.8; transition: 0.2s; }
        #ahg-panel{ position:fixed; bottom:20px; right:20px; left: auto; z-index:99999; background:#0f0f1e; border:1px solid #252545; border-radius:14px; min-width:260px; max-width:290px; font-family:'Segoe UI',sans-serif; color:#dde; box-shadow:0 8px 40px rgba(0,0,0,.7); max-height: calc(100vh - 40px); overflow: hidden; display:flex; flex-direction:column; }
        .a-tit{ background:linear-gradient(135deg,#3b2d82,#1e1b4b); color:#b9a9ff; font-weight:700; font-size:11px; letter-spacing:1.4px; text-transform:uppercase; padding:10px 14px 8px; border-radius:14px 14px 0 0; display:flex; align-items:center; gap:6px; cursor:grab; }
        .a-x{ margin-left:auto; cursor:pointer; opacity:.6; font-size:18px; }
        .a-body{ padding:10px 12px 12px; display:flex; flex-direction:column; gap:5px; overflow-y:auto; overflow-x:hidden; flex:1; }
        .a-row{ display:flex; justify-content:space-between; align-items:center; padding:5px 8px; border-radius:7px; background:rgba(255,255,255,.04); border-left:3px solid transparent; }
        ...
        .a-val.pos{color:#3ddc84;} .a-val.neg{color:#ff6b6b;} .a-val.warn{color:orange;} .a-val.neu{color:#b9a9ff;}
        ...
        .a-sec{ font-size:10px; letter-spacing:1px; text-transform:uppercase; color:#444466; padding:3px 0 1px; font-weight:700; }
        .a-foot{ font-size:10px; font-weight:800; color:#333355; text-align:right; padding:2px 14px 8px; }
        ...
        body.ahg-show-totals .ahg-day-total { display: block; }
        `;
    ...
    function criarEstrutura() {
        ...
        panel.addEventListener('mousedown', e => {
            if (!e.target.closest('.a-tit') || e.target.id === 'ahg-eye-toggle' || e.target.id === 'ahg-min') return;
            drag = true;
        ...
        try {
            ...
            <div class="a-body">
                ...
                <div class="a-row infos">
                    <span class="a-lbl">Situação</span>
                    <span class="a-val neu">${r.status}</span>
                </div>
                ${r.alerta ? `
                <div class="a-row danger">
                    <span class="a-lbl">Alerta</span>
                    <span class="a-val neg">${r.alerta}</span>
                </div>` : ''}

                <hr class="a-div">

                <div class="a-sec">Hoje</div>
                ${r.turno1 ? `
                <div class="a-row ${r.turno1.classe}">
                    <span class="a-lbl">1º turno</span>
                    <span class="a-val neu">
                        ${r.turno1.entrada} → ${r.turno1.saida}
                        <small>${fmtMin(r.turno1.total)} ${r.turno1.aberto ? '· em andamento' : ''}</small>
                    </span>
                </div>` : ''}
                ${r.turno2 ? `
                <div class="a-row ${r.turno2.classe}">
                    <span class="a-lbl">2º turno</span>
                    <span class="a-val neu">
                        ${r.turno2.entrada} → ${r.turno2.saida}
                        <small>${fmtMin(r.turno2.total)} ${r.turno2.aberto ? '· em andamento' : ''}</small>
                    </span>
                </div>` : ''}

                <div class="a-row infos">
                    <span class="a-lbl">Trabalhado</span>
                    <span class="a-val ${r.hoje.saldo >= 0 ? 'pos' : 'warn'}">${fmtMin(r.hoje.trabalhado)}</span>
                </div>
                <div class="a-row infos">
                    <span class="a-lbl">Saldo do dia</span>
                    <span class="a-val ${r.hoje.saldo >= 0 ? 'pos' : 'neg'}">${fmtMin(r.hoje.saldo)}</span>
                </div>

                <hr class="a-div">

                <div class="a-sec">Saídas</div>
                <div class="a-row warn" ><span class="a-lbl">⚠️ 6h</span><span class="a-val warn">${fmtHour(r.h6)}</span></div>
                <div class="a-row ok"><span class="a-lbl">✅ 8h</span><span class="a-val pos">${fmtHour(r.h8)}</span></div>
                <div class="a-row danger"><span class="a-lbl">⛔️ 10h</span><span class="a-val neg">${fmtHour(r.h10)}</span></div>
                <div class="a-row infos"><span class="a-lbl">🏆 Saída ideal</span><span class="a-val neu">${fmtHour(r.saidaIdeal)}</span></div>

                ${r.retornoMinimo ? `
                <hr class="a-div">
                <div class="a-sec">Intervalo</div>
                <div class="a-row infos"><span class="a-lbl">⏳ Retorno mínimo</span><span class="a-val neu">${fmtHour(r.retornoMinimo)}</span></div>
                <div class="a-row warn"><span class="a-lbl">⚠️ Retorno máximo</span><span class="a-val warn">${fmtHour(r.retornoMaximo)}</span></div>` : ''}

                <div class="a-row infos"><span class="a-lbl">🛌 Retorne depois das</span><span class="a-val neu">${fmtHour(r.retorno11h)}</span></div>

                <hr class="a-div">

                <div class="a-sec">Semanal — sem. ${getWeekNumber(new Date())}</div>
                <div class="a-row ${r.saldoSemana >= 0 ? 'ok' : 'warn'}">
                    <span class="a-lbl">Saldo semanal</span>
                    <span class="a-val ${r.saldoSemana >= 0 ? 'pos' : 'neg'}">${fmtMin(r.saldoSemana)}<small>${r.diasRegistrados} dias registrados</small></span>
                </div>

                <hr class="a-div">

                <div class="a-sec">Mensal</div>
                <div class="a-row ${r.saldoMes >= 0 ? 'ok' : 'warn'}">
                    <span class="a-lbl">Saldo mensal</span>
                    <span class="a-val ${r.saldoMes >= 0 ? 'pos' : 'neg'}">${fmtMin(r.saldoMes)}<small>${r.diasRestantesMes} úteis restantes</small></span>
                </div>
                <div class="a-row infos clickable" id="ahg-open-details">
                    <span class="a-lbl">📊 Horas realizadas</span>
                    <span class="a-val neu">${fmtMin(r.totalMes)}</span>
                </div>
            </div>

            <div class="a-foot">
                Atualizado ${fmtHour(nowMin())} · Reload em ${fmtCountdown(NEXT_REFRESH - Date.now())}
            </div>
            `;
    ...
    function abrirDetalhes(r) {
        ...
        box.style = `width:min(900px,95vw); max-height:90vh; overflow-y:auto; overflow-x:auto; background:#111827; border-radius:14px; padding:20px; color:#dde; font-family:Segoe UI,sans-serif;`;
        ...
        r.dias.filter(x => !x.isFuture && x.isBusinessDay).sort((a, b) => a.data - b.data).forEach((d, idx, arr) => {
            ...
            if (semana !== semanaAtual) {
                ...
                <table style="width:100%; border-collapse:collapse; margin-bottom:12px;">
                    <thead>
                        <tr>
                            <th style="text-align:left;padding:6px 4px;">Dia</th>
                            <th style="text-align:left;padding:6px 4px;">Data</th>
                            <th style="text-align:right;padding:6px 4px;">Horas</th>
            ...
            html += `
            <tr>
                <td>${diasSemana[d.data.getDay()]}</td>
                <td>${d.data.toLocaleDateString('pt-BR')}</td>
                <td align="right">${fmtMin(d.trabalhado)}</td>
                <td align="right">${fmtMin(d.saldo)}</td>
            </tr>`;
            ...
            if (!next || getWeekNumber(next.data) !== semana) {
                ...
                <tr style="background:#1f2937; font-weight:bold;">
                    ...
                    <td align="right">${fmtMin(totalSemana)}</td>
                    <td align="right">${fmtMin(saldoSemana)}</td>
                </tr>
                </tbody></table>`;
            }
        });
